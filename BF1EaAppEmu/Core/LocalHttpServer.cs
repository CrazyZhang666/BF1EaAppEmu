...
public static class LocalHttpServer
{
    ...
    public static void Run()
    {
        ...
        _httpListener.Prefixes.Add("http://127.0.0.1:10086/");
        _httpListener.Start();
        _httpListener.BeginGetContext(Result, null);

        LoggerHelper.Info("启动 Local Http 监听服务成功");
        LoggerHelper.Debug("Local Http 服务监听端口为 10086");
    }
    ...
    private static void Result(IAsyncResult asyncResult)
    {
        try
        {
            ...
            if (request.HttpMethod == "GET")
            {
                ...
                if (request.RawUrl.Equals("/CloseBf1Game", StringComparison.OrdinalIgnoreCase))
                {
                    WeakReferenceMessenger.Default.Send("", "CloseBf1Game");

                    response.OutputStream.Write(Encoding.UTF8.GetBytes("关闭战地1游戏成功"));
                    response.StatusCode = 200;
                    response.Close();
                }
            }
